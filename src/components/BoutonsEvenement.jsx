...
import { attestationDisponible } from '../utils/generateAttestationPDF'
...
// Un bouton indisponible reste visible (grise) avec la raison, pour que la personne sache quand il arrivera.

const TR = {
  fr: {
    titre: 'Mes documents COPAF',
    guide: 'Guide du participant', guideOk: 'Télécharger le PDF', guideNon: 'Bientôt disponible',
    fiche: 'Fiche de voyage individuelle', ficheOk: 'Télécharger le PDF', ficheNon: 'En préparation : elle apparaîtra dès que votre hébergement et vos transferts seront confirmés',
    programme: 'Programme', programmeOk: 'Sessions, horaires et intervenants',
    attestation: 'Attestation de participation', attestationOk: 'Télécharger le PDF',
    attestationPaiement: 'Disponible après confirmation de votre paiement', attestationDate: 'Disponible à partir du 20 octobre 2026, à la fin du Jour 2',
  ...
  en: {
    ...
    fiche: 'Individual travel sheet', ficheOk: 'Download the PDF', ficheNon: 'Being prepared: it will appear as soon as your accommodation and transfers are confirmed',
    programme: 'Programme', programmeOk: 'Sessions, schedule and speakers',
    attestation: 'Certificate of participation', attestationOk: 'Download the PDF',
    attestationPaiement: 'Available once your payment is confirmed', attestationDate: 'Available from 20 October 2026, at the end of Day 2',
...
export default function BoutonsEvenement({ lang, myDossier, voyage, onOpenProgramme }) {
  ...
  const telecharger = async genre => {
    ...
    try {
      if (genre === 'attestation') {
        const m = await import('../utils/generateAttestationPDF')
        await m.generateAttestationPDF({ personne: myDossier, lang: l, download: true })
      } else {
  ...
  const dateOk = attestationDisponible()
  const attestationOk = confirme && dateOk
  ...
  return (
    <Card icon="download" title={t.titre}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 10 }}>
        ...
        <Bouton
          icone="shield" titre={t.attestation} actif={attestationOk} chargement={gen === 'attestation'} onClick={() => telecharger('attestation')}
          sous={attestationOk ? t.attestationOk : (!confirme ? t.attestationPaiement : t.attestationDate)}
        />
      </div>
