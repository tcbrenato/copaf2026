-- mon_dossier() : remplace programme_personnalise (retire) par l'agenda en libre-service,
-- et ajoute l'historique des preuves de virement du participant.
drop function if exists public.mon_dossier();

create function public.mon_dossier()
 returns table(
...
begin
  ...
  select
    ...
    coalesce((
      select jsonb_agg(jsonb_build_object('id', p.id, 'url', p.url, 'statut', p.statut, 'commentaire_admin', p.commentaire_admin, 'created_at', p.created_at) order by p.created_at desc)
